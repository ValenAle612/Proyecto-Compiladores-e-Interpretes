...
public class GreaterBinaryExpNode extends BinaryExpressionNode {
    ...
    public GreaterBinaryExpNode(Token token){
        super(token);
    }
    ...
    public ConcreteType verify() throws SemanticException {
        ...
        if(left_side.verify().same_type(new IntType()) && right_side.verify().same_type(new IntType()))
            ...
            throw new SemanticException(token, "the binary operator "+token.getLexeme()+" is only for integer type");
